...
const SupperShop = () => {
    ...
    const handleMoreData = (slug:number) =>{
        router.push(`/interiors-faculty/${slug}`)
    }
    const supporShopData = cardData.filter(card=> card.id >=38 &&   card.id <= 44 )
  return (
    <section className="py-10 md:py-15">
    <MaxWidthWrapper>
        <SubHeading className="text-center text-gray-600 font-bold uppercase">  Supper Shop</SubHeading>
        <Paragraph className="text-center py-5 w-full md:w-[500px] flex mx-auto">
            You can choose your favourite supper shop from here. We can build your dream room at a minimum cost.
        </Paragraph>
        <Carousel>
            <CarouselContent> 
                        ...
                        <div className="border p-3 shadow-md mb-2 rounded-lg hover:scale-[102%] transition translate duration-500 h-full flex flex-col"> 
                            ...
                            <div className="flex justify-start w-full mt-auto"> 
                                <button
                                    ...
                                    onClick={()=> handleMoreData(card.slug)}
                                >
                                    More Details
                                </button>
                            </div>
